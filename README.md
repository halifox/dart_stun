...
- `initialRto`
  UDP 初始重传超时
- `backoffStrategy`
  UDP 重传退避策略，可选 `StunBackoffStrategy.linear` 和 `StunBackoffStrategy.exponential`，默认是 `linear`
- `maxRetransmissions`
  UDP 最大重传次数
...
如果你希望使用 RFC 5389 常见的“每次翻倍”节奏，可以设置 `backoffStrategy: StunBackoffStrategy.exponential`；如果不传，默认仍使用兼容旧版本行为的 `StunBackoffStrategy.linear`。
